...
class ProductService {
    async uploadImageService(req) {
        ...
        const result = await Promise.all(files.map(async (file) => {
            ...
            const newPath = path_1.default.resolve(direction_1.UPLOAD_IMAGE_DIR, `${newName}.jpg`);
    ...
    async addProduct(productReqBody, req) {
        const product = await database_service_1.default.products.insertOne(new Product_schema_1.Product({
    ...
    async uploadImagesProduct(req) {
        const url = await this.uploadImageService(req);
        return url;
    }
    async getProductList(queryParam) {
        const { name, brand, category, limit, page, color, cpu, ram, vga, laptop_category, operation_system, screen_frequency, screen_resolution, size_screen, style, touch_screen, sort, stock, price_min, price_max, other_filter } = queryParam;
        const filter = [];
        const sort_type = { new: 1 };
        ...
        const [productList, productListSize, total] = name
            ? await Promise.all([
                database_service_1.default.products
                    .aggregate([
                    {
                        $match: {
                            $text: {
    ...
    async getQuantity() {
        const [asus, acer, dell, hp, lenovo, msi, macbook, lg, microsoft, vaio, fashion, gaming, technology, common, black, gray, sliver, white, pink, gold, blue, laptopGaming, doHoaKienTruc, phoThongVanPhong, mongNheThoiTrang, doanhNhan, intelCeleronPentium, intelCorei3, intelCorei5, intelCorei7, intelCorei9, amdRyzen3, amdRyzen5, amdRyzen7, appleM1, appleM2, appleM3, vgaNvidia, vgaAmd, vagTichHop, rtx2050_2050ti, rtx4050, rtx3050_3050ti, rxt4060, s_13_3inch, s_13inch, s_13_5inch, s_13_6inch, s_13_4inch, s_15_4inch, s_14inch, s_14_2inch, s_15inch, s_14_5inch, s_15_6inch, s_16inch, s_16_2inch, s_17inch, s_15_3inch, s_other, r_HD_1366x768, r_Full_HD_1920x1080, r_WUXGA_1920x1200, r_WQHD_2560x1440, r_WQXGA_2560x1600, r_Pixel_Sense_2736x1824, r_2_8K_2880x1800, r_Retina_2560x1600, r_Retina_2560x1664, r_4K_3840x2160, r_3k_3200x2000, r_Retina_2880x1864, r_other, t_yes, t_no, f_60Hz, f_90Hz, f_120Hz, f_165Hz, os_windows, os_linux, os_dos, os_macos, os_ubuntu, ram_4GB, ram_8GB, ram_16GB, ram_32GB, ram_gt_32GB] = await Promise.all([
            database_service_1.default.products
                .aggregate([
                {
                    $lookup: {
    ...
    async getProductDetail(id) {
        await database_service_1.default.products.updateOne({
        ...
        const product = await database_service_1.default.products
            .aggregate([
            {
                $match: {
                    _id: new mongodb_1.ObjectId(id)
    ...
    async searchProduct(name) {
        const productList = await database_service_1.default.products
            .find({
            $text: {
                $search: `'"${name}"'`
            }
